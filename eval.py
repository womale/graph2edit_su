...
def main():
    ...
    checkpoint = torch.load(os.path.join(exp_dir, 'epoch_51.pt'))
    ...
    with open(pred_file, 'a') as fp:
        for idx in p_bar:
            ...
            for beam_idx, path in enumerate(top_k_results):
                ...
                if correct and not beam_matched:
                    accuary_matrix[rxn_class][beam_idx]+=1
                    top_k[beam_idx] += 1
                    beam_matched = True
        ...
        for idx in range(10):
            ...
            for top_num in [1,3,5,10]:
                print("top%d accuary: %f" %(top_num, np.sum(accuary_matrix[idx][:top_num])/class_num[idx]), end="  ")
            print("sample number %d" %(class_num[idx]))
